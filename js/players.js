...
const Players = {
      ...
      <div class="player-row" style="flex-wrap:wrap;">
        ...
        <div class="player-info" style="flex:1;">
          ...
          <div class="player-meta">
            HCP ${p.hcp} · Quota ${p.quota} · ${p.tee} tee
            ${p.email ? ' · ' + p.email : ''}
            ${p.phone ? ' · ' + p.phone : ''}
            ${p.ghin ? ' · GHIN: ' + p.ghin : ''}
          </div>
      ...
      <div style="background:white;border-radius:20px;padding:24px;width:100%;max-width:360px;margin:auto;">
        ...
        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-bottom:14px;">
          <div class="form-group" style="margin-bottom:0;">
            ...
            <input class="form-input" type="text" id="edit-first" value="${p.first||''}" />
          </div>
          <div class="form-group" style="margin-bottom:0;">
            <label class="form-label">Last name</label>
            <input class="form-input" type="text" id="edit-last" value="${p.last||''}" />
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Email</label>
          <input class="form-input" type="email" id="edit-email" value="${p.email||''}" />
        </div>
        <div class="form-group">
          <label class="form-label">Phone</label>
          <input class="form-input" type="tel" id="edit-phone" value="${p.phone||''}" />
        </div>
        <div class="form-group">
          <label class="form-label">GHIN number</label>
          <input class="form-input" type="text" id="edit-ghin" value="${p.ghin||''}" />
        </div>
        <div class="form-group">
          <label class="form-label">Handicap index</label>
          <input class="form-input" type="number" id="edit-hcp" value="${p.hcp||18}" step="0.1" min="0" max="54" />
        </div>
        ${isAdmin ? `
        <div class="form-group">
          <label class="form-label">Quota <span style="font-size:10px;color:var(--amber);background:var(--amber-light);padding:2px 6px;border-radius:10px;">Admin only</span></label>
          <input class="form-input" type="number" id="edit-quota" value="${p.quota||18}" step="1" min="0" />
        </div>
        <div class="form-group">
          <label class="form-label">Preferred tee <span style="font-size:10px;color:var(--amber);background:var(--amber-light);padding:2px 6px;border-radius:10px;">Admin only</span></label>
          <div class="tee-row" id="edit-tee-row">
            ${['Black','Blue','White','Red'].map(t => `<button type="button" class="tee-btn${p.tee===t?' tee-active-'+t.toLowerCase():''}" data-tee="${t}" onclick="Players.editSelectTee(this)">${t}</button>`).join('')}
          </div>
        </div>` : `
        <div class="form-group">
          <label class="form-label">Quota</label>
          <div class="form-input" style="background:var(--bg-2);color:var(--text-2);">${p.quota} <span style="font-size:11px;">(set by admin)</span></div>
        </div>`}
  ...
  async makeAdmin(id) {
    if (!confirm('Make this player an admin? They will have full access to manage the group.')) return;
    await DB.updatePlayer(id, { isAdmin: true });
    // Also update their Firebase user profile
    const p = this.list.find(x => x.id === id);
  ...
  async save() {
    ...
    const player = {
      ...
      email, phone, ghin, hcp, quota,
      tee: this.selectedTee,
      history: [], linkedUid: null,
  ...
  resetForm() {
    ['add-first','add-last','add-email','add-phone','add-ghin','add-hcp','add-quota'].forEach(id => {
